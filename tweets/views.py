...
class TweetView(APIView):
    ...
    def get(self, request, format=None):
        week_previous = datetime.now(pytz.timezone(settings.TIME_ZONE)) - timedelta(days=7)
        ...
        most_retweeted = Tweet.objects.raw("""select distinct on (date(created_at)) retweet_count,
